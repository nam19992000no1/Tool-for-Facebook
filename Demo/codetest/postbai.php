...
$typelove = $obj->$id->reactions->data->type;
$countcomment = count($comment);
$countreaction = count($reaction);

//$countlove = count(array_keys($typelove, "LOVE"));
...
$url = 'https://graph.facebook.com/v2.8/735969126506947?fields=members.limit(1000).summary(total_count),member_request_count&access_token='.$facebook_access_token;
$ch  = curl_init();
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_URL, $url);
...
// -- Ket thuc --
// -- Bat dau review bai viet moi nhat cua group --
$url = 'https://graph.facebook.com/v2.8/?ids='.$idlatestpost.'?fields=from,reactions.summary(total_count)&access_token='.$facebook_access_token;
$ch  = curl_init();
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_URL, $url);
...
curl_close($ch);
$obj = json_decode($result);
$iidlatestpost = $obj->$idlatestpost->from->id;
$namelatestpost = $obj->$idlatestpost->from->name;
$reactionlatestpost = $obj->$idlatestpost->reactions->summary->total_count;
...
{$countangry} bạn đã tức giận, bạn @[{$iidangry}:0.{$nameangry}:0]
